(function () {
  ...
  function applyCardMeta(order, hidden, custom) {
    ...
    requestAnimationFrame(function() {
      window.dispatchEvent(new Event('sticky-cards-changed'));
    });
  }
  ...
  function initStickyCards() {
    ...
    // Cache per-stack meta to avoid repeated DOM queries during scroll.
    // Shape: [{ stack, wrapper, scenes, N, transitions, total, vh }]
    var meta = [];

    function collectScenes(stack) {
    ...
    // Full setup: recompute heights + re-cache. Runs on init and on width changes.
    function setup() {
      var vh = window.innerHeight;
      meta = stacks.map(function (stack) {
        var wrapper = stack.querySelector('.cards-wrapper');
        if (!wrapper) return null;
        var scenes = collectScenes(stack);
        var N = scenes.length;
        if (!N) return null;
        var transitions = Math.max(1, N - 1);
        var total = transitions + 1;
        wrapper.style.height = (total * vh) + 'px';
        // Prime transforms (card 0 in place, rest below)
        scenes.forEach(function (s, i) {
          s.style.transform = i === 0 ? 'translate3d(0,0,0)' : 'translate3d(0,100%,0)';
          s.style.zIndex = String(i + 1);
        });
        return { stack: stack, wrapper: wrapper, scenes: scenes, N: N, total: total, vh: vh };
      }).filter(Boolean);
      update();
    }
    ...
    function update() {
      ...
      ticking = true;
      requestAnimationFrame(function () {
        // READ phase — all layout reads first (no interleaved writes → no forced reflow)
        var frames = [];
        for (var m = 0; m < meta.length; m++) {
          var info = meta[m];
          var rectTop = info.wrapper.getBoundingClientRect().top;
          var scrollable = info.wrapper.offsetHeight - info.vh;
          var progress = scrollable > 0 ? Math.max(0, Math.min(1, -rectTop / scrollable)) : 0;
          frames.push(progress * info.total);
        }
        // WRITE phase — apply transforms, no layout reads
        for (var k = 0; k < meta.length; k++) {
          var info2 = meta[k];
          var p = frames[k];
          var scenes = info2.scenes;
          for (var i = 0; i < scenes.length; i++) {
            if (i === 0) continue; // first card never moves, no writes needed
            var local = p - (i - 1);
            var ty = local <= 0 ? 100 : local < 1 ? (1 - easeOut(local)) * 100 : 0;
            scenes[i].style.transform = 'translate3d(0,' + ty + '%,0)';
          }
        }
        ticking = false;
      });
    ...
    // CMS fires this after hiding/showing scenes
    window.addEventListener('sticky-cards-changed', setup);

    // Only re-setup on width changes. Mobile browsers fire resize when the URL
    // bar shows/hides, which would otherwise reset transforms mid-scroll and jitter.
